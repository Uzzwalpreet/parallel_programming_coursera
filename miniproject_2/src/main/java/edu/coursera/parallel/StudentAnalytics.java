...
public final class StudentAnalytics {
	...
	public double averageAgeOfEnrolledStudentsParallelStream(final Student[] studentArray) {
		...
		return Stream.of(studentArray).parallel().filter(student -> student.checkIsCurrent())
				.mapToDouble(s -> s.getAge()).average().orElse(0);
	...
	public String mostCommonFirstNameOfInactiveStudentsParallelStream(final Student[] studentArray) {
		// throw new UnsupportedOperationException();
		return Stream.of(studentArray).parallel().filter(student -> !student.checkIsCurrent())
				.collect(Collectors.groupingBy(Student::getFirstName, Collectors.counting())).entrySet()
				.parallelStream().max(Comparator.comparing(Map.Entry::getValue)).get().getKey();
	}
